...
class DeepNeuralNetwork:
    ...
    def build_graph(self):
        ...
        with self._graph.as_default():
            ...
            for i in range(len(self.layers_conf)):
                if i == 0:
                    mat = tf.Variable(tf.random_normal([self.input_dim, self.layers_conf[i]]))
                else:
                ...
                prob = tf.nn.relu(tf.add(tf.matmul(self.probs_matrix[-1], mat), bias))
                self.probs_matrix.append(prob)

            mat = tf.Variable(tf.random_normal([self.layers_conf[-1], self.output_dim]))
            ...
            self.loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=one_hot_labels))
            optimizer = tf.train.AdamOptimizer()
            self.train_op = optimizer.minimize(self.loss_op)

            # Test accuracy
            self.acc, self.acc_op = tf.metrics.accuracy(labels=self.labels, predictions=self.predicts)
    ...
    def get_accuracy(self, sess, test_examples, test_labels):
        _, cur_acc = sess.run([self.acc_op, self.acc], feed_dict={self.inputs: test_examples, self.labels: test_labels})
        return cur_acc
